"""
RNN + Bayesian Network Hybrid Model
=====================================
Vanilla Recurrent Neural Network applied to tabular loan data
...
class _RNNNetwork(nn.Module):
    """Vanilla RNN that treats each feature as a time-step of length 1."""
    ...
    def forward(self, x):
        x = x.unsqueeze(-1)                # (batch, seq=features, 1)
...
class RNNBayesianHybrid:
    ...
    def __init__(self, input_dim: int, hidden_size: int = 64,
        ...
        self.network = _RNNNetwork(input_dim, hidden_size, num_layers, dropout).to(self.device)
